...
def extract_kpis(project_id: str, document_id: str, document_text: str) -> dict:
    ...
    chunks = search_in_document(project_id, document_id, "indicateurs performance objectifs mesures résultats")
...
def extract_kpis_multi(project_id: str, document_ids: list, document_texts: list) -> dict:
    """Extraction KPI sur plusieurs documents cibles."""
    prompt_path = os.path.join(BASE_DIR, '..', 'prompts', 'kpi_prompt.txt')
    try:
        with open(prompt_path, 'r', encoding='utf-8') as f:
            prompt_template = f.read()
    ...
    chunks = search_in_documents(project_id, document_ids, "indicateurs performance objectifs mesures résultats")
    context_text = "\n".join(chunks) if chunks else "Aucun contexte disponible"
    ...
    try:
        ...
        logger.error(f"Erreur parsing JSON KPI multi-docs : {e}\nRéponse brute: {reponse}")
        return {"kpis": [], "resume": "Erreur de parsing JSON", "raw": reponse}