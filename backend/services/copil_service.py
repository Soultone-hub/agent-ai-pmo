...
def generate_copil(project_id: str, document_id: str, document_text: str) -> dict:
    ...
    chunks = search_in_document(project_id, document_id, "avancement décisions actions jalons planning budget")
...
def generate_copil_multi(project_id: str, document_ids: list, document_texts: list) -> dict:
    """Génération COPIL sur plusieurs documents cibles."""
    prompt_path = os.path.join(BASE_DIR, '..', 'prompts', 'copil_prompt.txt')
    try:
        with open(prompt_path, 'r', encoding='utf-8') as f:
            prompt_template = f.read()
    ...
    chunks = search_in_documents(project_id, document_ids, "avancement décisions actions jalons planning budget")
    context_text = "\n".join(chunks) if chunks else "Aucun contexte disponible"
    ...
    try:
        ...
        logger.error(f"Erreur parsing JSON COPIL multi-docs : {e}\nRéponse brute: {reponse}")
        return {"error": "Erreur de parsing JSON", "raw": reponse}